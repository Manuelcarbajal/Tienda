...
async function read() {
  const response = await sheets.spreadsheets.values.get({
    spreadsheetId: "1CfgsQ5KX8PZTjuleBfoNh9LcFJ5-ajUZmdRlShkfnqA",
    range: "Products!A2:I",
  });
...
async function write(products) {
  let values = products.map((p) => [p.id, p.name, p.price, p.image, p.stock, p.category,p.tipo,p.sabor,p.contenido]);
